...
| `chrony_*` (`driftfile`, `makestep`, `rtcsync`, `logdir`, `minpoll`, `maxpoll`, `keyfile`, `leapsectz`, `ptp_kvm`) | Ansible / group_vars/all.yml (empty poll/keyfile/leapsectz omit the directive; `ptp_kvm` false stops adding the KVM PTP refclock and leaves its module and udev files in place) |
