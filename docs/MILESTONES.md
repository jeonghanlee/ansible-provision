...
| NFS bake fix | Drop app_ioc_runner from `04_nfs_sim`; default stdout callback | Carry-forward | Open (review) | `3ea5c20` removes app_ioc_runner from `playbooks/04_nfs_sim.yml` (become root cannot chown/rm/execve inside the root_squash nfs_sim mount during the the site bake host golden bake, so every bake failed); `3ccc8b8` sets the stdout callback to default for ansible-core-only hosts. Review pending: confirm the removal drops no intended NFS source-root coverage (cf. the local/NFS split finding below), and weigh a `requirements.yml` collection install over the callback change. Origin: epics-ioc-runner M19 V002 bake (2026-06-26). |
